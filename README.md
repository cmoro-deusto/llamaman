...
If the AUR is down or the automated push is paused, the release workflow still generates a PKGBUILD and publishes it as a GitHub Actions artifact named `llamaman-dist-<tag>`. You can build the package manually:
```sh
# Download the artifact from the release Actions run, e.g.
# https://github.com/cmoro-deusto/llamaman/actions
wget -O llamaman-bin.tar.gz <artifact-url>
tar -xzf llamaman-bin.tar.gz
cd llamaman-bin
makepkg -si
```
The artifact contains the generated `PKGBUILD`, `.SRCINFO`, and the source tarball used by the AUR package. This is a temporary fallback while AUR publishing is disabled.
