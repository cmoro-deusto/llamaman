...
func TestParseLoadPhase(t *testing.T) {
	...
	cases := []struct {
		...
		{"llama_model_loader: loading model tensors, this can take a while...", "loading model", nil},
		{"loading model from /m/model.gguf", "loading model", nil},
		{"llama_model_loader: loading model file with 2 tensors", "loading model", nil},
		// Real b10281 output, HF download path (owner's log):
		{"0.00.625.308 I srv    load_model: loading model 'unsloth/Qwen3.6-27B-GGUF:UD-Q4_K_XL'", "loading model", nil},
		{"0.03.291.940 I srv    load_model: initializing, n_slots = 1, n_ctx_slot = 150016", "initializing", nil},
		{"0.03.294.882 I srv  llama_server: model loaded", "model loaded", nil},
